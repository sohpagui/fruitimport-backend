...
export async function ventesSetDernierJours() {
  ...
  const groupes: Record<string, Record<number, number>> = {}
  for (const vente of ventes) {
    const jour = vente.date.toISOString().split('T')[0]
    if (!groupes[jour]) groupes[jour] = {}
    if (!groupes[jour][vente.agenceId]) groupes[jour][vente.agenceId] = 0
...
export async function ventesParFruit(agenceId?: number) {
  const where: any = {
    commande: {
      statut: { in: ['CONFIRMEE', 'PREPAREE', 'EN_LIVRAISON', 'LIVREE'] },
    },
  }
  if (agenceId) where.commande.agenceId = agenceId
...
export async function statsParAgence(agenceId: number) {
  const aujourd_hui = new Date()
  aujourd_hui.setHours(0, 0, 0, 0)
  const demain = new Date(aujourd_hui)
  demain.setDate(demain.getDate() + 1)
...
export async function genererSynthese(agenceId1: number, agenceId2: number) {
