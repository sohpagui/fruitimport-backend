...
export async function listerClients(params: {
  ...
  if (params.statutCredit) where.statutCredit = params.statutCredit
...
export async function trouverClientParId(id: number) {
...
export async function modifierLimiteCredit(
  clientId: number,
  limiteCredit: number,
  modifiePar: number
) {
...
export async function enregistrerPaiementCredit(data: {
  clientId: number
  commandeId?: number
  montant: number
...
async function recalculerStatutCredit(clientId: number) {
  ...
  let nouveauStatut: StatutCredit = 'EN_REGLE'
