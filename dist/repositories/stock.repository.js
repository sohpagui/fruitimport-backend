...
exports.receptionnerMarchandise = receptionnerMarchandise;
...
async function listerStocks(params) {
    ...
    if (params.fruitId)
        where.fruitId = params.fruitId;
    if (params.calibreId)
        where.calibreId = params.calibreId;
    if (params.categorie)
...
async function obtenirAlertesStock(agenceId) {
...
async function deduireStock(tx, agenceId, fruitId, calibreId, categorie, quantite) {
