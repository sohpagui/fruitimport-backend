...
exports.genererRapportJournalier = genererRapportJournalier;
exports.genererRapportJournalierBuffer = genererRapportJournalierBuffer;
...
async function genererRapportJournalier() {
    ...
    const [agences, commandesJour, livraisonsJour, pertes, alertesStock, clientsRetard] = await Promise.all([
        prisma_1.default.agence.findMany(),
        ...
        prisma_1.default.perte.findMany({
            where: { date: { gte: aujourd_hui, lt: demain } },
    ...
    return new Promise(async (resolve, reject) => {
        ...
        agences.forEach(agence => {
            ...
            if (cmdsAgence.length === 0)
                doc.text('  Aucune commande aujourd hui.');
            doc.moveDown(0.5);
        });
        // Livraisons
        doc.addPage();
        ...
        if (livraisonsJour.length === 0)
            doc.text('Aucune livraison aujourd hui.');
        doc.moveDown();
        // Pertes
        ...
        if (pertes.length === 0)
            doc.text('Aucune perte enregistree aujourd hui.');
        doc.moveDown();
        // Stocks bas
        ...
        if (clientsRetard.length === 0)
            doc.text('Aucun client en retard.');
        doc.moveDown();
...
async function genererRapportJournalierBuffer() {
    const aujourd_hui = new Date();
    aujourd_hui.setHours(0, 0, 0, 0);
    const demain = new Date(aujourd_hui);
    demain.setDate(demain.getDate() + 1);
    ...
    const [agences, commandesJour, livraisonsJour, pertes, alertesStock, clientsRetard] = await Promise.all([
        prisma_1.default.agence.findMany(),
        prisma_1.default.commande.findMany({ where: { date: { gte: aujourd_hui, lt: demain } }, include: { client: true, agence: true } }),
        prisma_1.default.livraison.findMany({ where: { dateAssignation: { gte: aujourd_hui, lt: demain } }, include: { commande: { include: { client: true } }, livreur: true } }),
        prisma_1.default.perte.findMany({ where: { date: { gte: aujourd_hui, lt: demain } }, include: { fruit: true, agence: true } }),
        prisma_1.default.stock.findMany({ where: { quantiteCartons: { lte: 5 } }, include: { fruit: true, agence: true, calibre: true } }),
        prisma_1.default.client.findMany({ where: { statutCredit: 'EN_RETARD' }, include: { agence: true } })
    ]);
    const totalVentes = commandesJour.reduce((sum, c) => sum + Number(c.montantTotal), 0);
    const totalPertes = pertes.reduce((sum, p) => sum + Number(p.valeurPerdue), 0);
